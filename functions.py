...
class Initialize_map:
    ...
    def create_blank_map(self):
        ...
        numberliste = 0
        while numberliste < (self.player.x+1):
            self.player.full_map[numberliste] = [""]*(self.player.y+1)
            numberliste +=1
...
class Move_map:
    ...
    def movement(self, direction):
        if direction == "s":
            ...
            x +=1
            if self.check_position(x,y) == False:
                x -=1
    ...
    def check_position(self,position_x, position_y):
        if position_x < 0 or position_x > self.player.x or position_y < 0 or position_y > self.player.y:
